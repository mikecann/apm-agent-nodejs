...
const { LambdaExtensionTransport } = require('./lambda-extension-transport')
...
class Config {
  constructor (opts) {
    ...
    if (this.disableSend) {
      ...
      this.transport = function lambdaExtensionTransaction(conf, agent) {
        const config = getBaseClientConfig(conf, agent)
        var transport = new LambdaExtensionTransport(config)
      }
