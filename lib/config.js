...
class Config {
  constructor (opts) {
    ...
    } else if ('aws-lambda' === this.serverless) {
      this.transport = function httpTransport(conf, agent) {
        const baseClientConfig = getBaseClientConfig(conf, version, config, userAgent, agent)
        return new SocketTransport(baseClientConfig)
      }
...
function getBaseClientConfig(conf, version, config, userAgent, agent) {
